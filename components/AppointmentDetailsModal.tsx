...
import { X, Calendar, Clock, MapPin, Truck, User, FileText, Package, Briefcase, Info, List, CheckCircle2, AlertTriangle } from 'lucide-react';
...
export const AppointmentDetailsModal: React.FC<AppointmentDetailsModalProps> = ({ isOpen, onClose, appointmentId }) => {
    const { appointments, docks, yardSlots, carriers, formatDate, formatDateTime, updateAppointment, addToast } = useData();
                ...
                <div className="bg-white dark:bg-[#1e1e1e] w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden flex flex-col max-h-[90vh]">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                            ...
                            {appt.acknowledgementStatus === 'RescheduleSuggested' && appt.suggestedStartTime && (
                                <div className="col-span-1 md:col-span-2 bg-blue-500/10 border border-blue-500/20 rounded-2xl p-5 space-y-4 animate-in slide-in-from-top-2 duration-300">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-full bg-blue-500/20 flex items-center justify-center">
                                            <Clock className="w-5 h-5 text-blue-500" />
                                        </div>
                                        <div>
                                            <h4 className="text-sm font-black text-blue-600 dark:text-blue-400 uppercase tracking-tight">New Timing Suggested</h4>
                                            <p className="text-xs text-slate-500 dark:text-gray-400">The facility has suggested an alternate slot for this appointment.</p>
                                        </div>
                                    </div>

                                    <div className="flex flex-col md:flex-row gap-4 items-start md:items-center bg-white dark:bg-black/20 p-4 rounded-xl border border-blue-100 dark:border-blue-900/30">
                                        <div className="flex-1">
                                            <p className="text-[10px] uppercase font-black text-slate-400 mb-1">Proposed Slot</p>
                                            <div className="text-lg font-bold text-slate-900 dark:text-white">
                                                {new Date(appt.suggestedStartTime).toLocaleString(undefined, {
                                                    weekday: 'short',
                                                    month: 'short',
                                                    day: 'numeric',
                                                    hour: '2-digit',
                                        ...
                                        <div className="flex gap-2 w-full md:w-auto">
                                            <button
                                                onClick={async () => {
                                                    await updateAppointment(appt.id, {
                                                        startTime: appt.suggestedStartTime,
                                                        status: 'Scheduled',
                                                        acknowledgementStatus: 'Confirmed',
                                                        suggestedStartTime: null // Clear suggestion
                                                    });
                                                    addToast('Suggestion Accepted', 'The appointment has been rescheduled and confirmed.', 'success');
                                                    onClose();
                                                }}
                                                className="flex-1 md:flex-none px-4 py-2 bg-emerald-500 text-white rounded-lg text-xs font-bold flex items-center justify-center gap-2 hover:bg-emerald-600 transition-colors"
                                            >
                                            ...
                                            <button
                                                onClick={async () => {
                                                    ...
                                                    addToast('Suggestion Rejected', 'The appointment request has been cancelled.', 'info');
                                                    onClose();
                                                }}
                                                className="flex-1 md:flex-none px-4 py-2 bg-red-500 text-white rounded-lg text-xs font-bold flex items-center justify-center gap-2 hover:bg-red-600 transition-colors"
                                            >
